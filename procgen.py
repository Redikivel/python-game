...
class RectangularRoom:
    ...
    def inner(self) -> Tuple[slice, slice]:
        """Returns the inner area of this room as a 2D array"""
...
def tunnel_between(start: Tuple[int, int], end: Tuple[int, int]) -> Iterator[Tuple[int, int]]:
    """Returna an L-shaped tunnel between these two points"""
    x1, y1 = start
    x2, y2 = end
    if random.random() < .5:
...
def generate_dungeon(max_rooms: int, room_min_size: int, room_max_size: int,
                     map_width: int, map_height: int, max_monsters_per_room: int, player: Entity,) -> GameMap:
